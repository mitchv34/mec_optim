...
# ╔═╡ addc8c70-1021-4aab-8512-82a31ca54343
using VoronoiCells, GeometryBasics, Plots, Random

# ╔═╡ 54df42c1-a3c6-4855-a2bc-647a24d4dc2d
using PyCall

# ╔═╡ 0e3e3cd2-72fd-11ec-3190-e367bf10b05f
html"""
<h1> <center>Block 2b: Semi-discrete optimal transport</center> </h1>
...
# ╔═╡ 4bd171cd-104e-4a26-af5e-89af85c760f1
md"""
## References
...
# ╔═╡ 1720ef7c-9f58-4506-be43-3c1c2e9358d8
md"""
# Motivation
...
# ╔═╡ bc3cfac0-e704-4c2c-8694-3c29857ac6c7
md"""
### A reformulation
...
$$\begin{align*}
u\left(  x\right)  =\tilde{u}\left(  x\right)  +\left\vert x\right\vert ^{2}/2\text{, and }v_{j}=\tilde{v}_{j}+\left\vert y_{j}\right\vert ^{2}/2,
\end{align*}$$
...
# ╔═╡ a2abc506-ab99-4165-9ea3-6ceeca6e6e14
md"""
...
# ╔═╡ 4cb5b953-8f86-495a-93b5-85f163ac7ea8
md"""**Example**. We will generate a Voronoi tesselations where $10$ fountains are distributed uniformly on $[0,1]^2$, and $\tilde{v} = 0$."""

# ╔═╡ 719da30d-32f4-4f4b-b323-ec6a722f2a5a
begin
	rng = Random.seed!(1200)
	rect = Rectangle(Point2(0, 0), Point2(1, 1))
	points = [Point2(rand(rng), rand(rng)) for _ in 1:10]
end

# ╔═╡ 0f75f85d-1cc0-457d-88be-583e346be967
tess = voronoicells(points, rect);

# ╔═╡ b4d20018-02cf-404a-a97f-eda5dfedf8e6
tess.Cells[1]

# ╔═╡ cebe8b51-818d-40a7-afa9-b07423cfef51
begin
	scatter(points, markersize = 4, label = "", markershape=:cross, size = (550, 550),  framestyle=:none)
	plot!(tess, legend = :topleft, label="")
	centroids = sum.(tess.Cells) ./ length.(tess.Cells)
	scatter!(centroids,  markersize = 4, label = "",markershape=:circle)
	annotate!([(centroids[n][1] + 0.018, centroids[n][2] + 0.018, Plots.text(n, 12, "Computer Modern")) for n in 1:10])
end

# ╔═╡ 82a3f0a2-f388-44d7-b505-65b6dc69cf33
md"""
...
# ╔═╡ fa09c91e-b27f-491d-a7df-10c2d15fc218
md"""
...
# ╔═╡ 8f98682d-2c6d-4f7d-9c28-b632efa5c33c
md"""
## Duality
...
# ╔═╡ 47d33c29-65c0-4838-91c2-a025853b758a
md"""
### Splitting the mass
...
# ╔═╡ 38a4a972-0ebf-40e9-bd08-8270e47640cf
md"""
...
# ╔═╡ a5252286-f7d0-43d1-b95d-50f98261fbe8
begin
	pysdot = pyimport("pysdot") 
	np = pyimport("numpy")
end

# ╔═╡ e8148f01-59f0-41d7-906d-5b605c28a51b
Ys = np.reshape(np.random.uniform(0,1,2*10), (-1,2))

# ╔═╡ a882eb85-74df-43b4-8043-733191041ec3
vor_dia = pysdot.PowerDiagram(Ys)

# ╔═╡ 9002dea0-83b2-47a0-bdd7-01d31ca01abf
vor_dia.domain._inst

# ╔═╡ 2d0a31fe-e247-44e3-9907-7bd1bf1bd41e
vor_dia._updated_grid()

# ╔═╡ eff07bee-26f6-47c4-8d18-3b9be83f919a
tess.Generators
